...
client_id = '215912641321-53iu30j7st6jkh95movshvfsa599n1qp.apps.googleusercontent.com'
client_secret = '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'
...
def callback():
    ...
    # Obtém o email do usuário a partir do token de acesso
    id_token_string = token.get('id_token')

    # Valida e decodifica o token utilizando a chave pública
    id_info = id_token.verify_oauth2_token(
        id_token_string, requests.Request(), client_id)

    # Obtém o email do usuário do dicionário id_info
    email = id_info['email']



    # Obtém apenas o access token do dicionário token
    access_token = token['access_token']

    #Grava o token num ficheiro json token.json
    grava_token(token)

    return f'{email} logged in successfully. Token {access_token} gravado!'
...
if __name__ == '__main__':
    ...
    context.load_cert_chain(certfile='../certs/serv.crt',
                            keyfile='../certs/serv.key')
    app.run('localhost', ssl_context=context, port= 5000, debug=True)
